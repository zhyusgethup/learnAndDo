...
//        getServerAndChannel(request,null);
    ...
    public ResultBean addNotice(@Param("content") String content,@Param("state")Integer state,@Param("order")Integer order) {
        try {
            Map<String, String> map = new HashMap<>();
//            if(!StringUtils.isEmpty(token))
                map.put("token","dasdsad");
            if(null != state)
            ...
            HttpUtil.loginServerReturnVoidPost(HttpPropertiesLoader.O_ADD_NOTICE_URL, map);
    ...
    @At("/modNotice")
    @Ok("json:{ignoreNull:true}")
    public ResultBean modNotice(@Param("content")String content,@Param("state") Integer state,@Param("order") Integer order,@Param("id") Integer id){
        try {
            Map<String, String> map = new HashMap<>();
            if(!StringUtils.isEmpty(content))
                map.put("content",content);
            ...
            if(map.isEmpty())
                throw new GmException("参数错误");
            HttpUtil.loginServerReturnVoidPost(HttpPropertiesLoader.O_UPDATE_NOTICE_URL, map);
            return ResultBean.getEmptyBean();
        }catch (GmException e){
            return new ResultBean(e);
        }catch (Exception e){
    ...
    @At("/outPlayer")
    @Ok("json:{ignoreNull:true}")
    public ResultBean getNotice(@Param("state")Integer state){
        try {
            Map<String, String> map = new HashMap<>();
            if(null != state)
                map.put("state",String.valueOf(state));
            if(map.isEmpty())
                throw new GmException("参数错误");
            Object data = HttpUtil.loginServerReturnObjectPost(HttpPropertiesLoader.O_GET_NOTICE_URL, map);
