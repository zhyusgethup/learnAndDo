...
for f in m2-lib/*.jar; do
  CLASSPATH=${CLASSPATH}:$f;
done

# figure out which class to run
CLASS='com.cellsgame.game.Bootstrap'
...
# GC LOG
JAVA_OPTS=$JAVA_OPTS" -Xnoclassgc -verbose:gc -Xloggc:gc.log -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+PrintGCApplicationConcurrentTime -XX:+PrintGCApplicationStoppedTime"
# Heap Dump (Memory Analyzer)
# jmap -dump:format=b,file=$PROJECT_HOME/logs/heamdump.out
JAVA_OPTS=$JAVA_OPTS" -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=logs/dump.hprof"
# DEV ENV
